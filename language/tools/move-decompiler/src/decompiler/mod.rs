...
use move_model::{addr_to_big_uint, model::{FunctionEnv, GlobalEnv, ModuleEnv, StructEnv}, ty::{PrimitiveType, ReferenceKind, Type}};
    ...
    fn module_for_binary(&self, binary: &BinaryIndexedView) -> ModuleEnv<'_> {
        match binary {
            BinaryIndexedView::Module(compiled) => {
                ...
                let this_module_addr = addr_to_big_uint(&compiled.address());
        ...
        for binary in self.binaries.clone() {
            ...
            if is_script {
                ...
                result.add_line(format!(
                    "module {} {{",
                    module.get_name().display_full(&module.symbol_pool())
                ));
            }
