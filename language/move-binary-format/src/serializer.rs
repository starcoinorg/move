...
impl CompiledScript {
    ...
    pub fn serialize(&self, binary: &mut Vec<u8>) -> Result<()> {
        self.serialize_to_version(binary, Some(VERSION_MAX))
    }
    pub fn serialize_to_version(
        ...
        let mut ser = ScriptSerializer::new(major_version.unwrap_or(VERSION_MAX));
...
impl CompiledModule {
    ...
    pub fn serialize_to_version(
        ...
        let mut binary_data = BinaryData::from(binary.clone());
        let mut ser = ModuleSerializer::new(major_version.unwrap_or(VERSION_MAX));
        let mut temp = BinaryData::new();
