...
pub fn taskify<Command: Debug + Parser>(filename: &Path) -> Result<Vec<LazyParseTaskInput<Command>>> {
    use regex::Regex;
    use std::{
        fs::File,
    ...
    for (number, (commands, text)) in bucketed_lines.into_iter().enumerate() {
        ...
        //
        // Optimized by Starcoin contributor
        // =====================================
...
pub struct PrintBytecodeCommand {
    ...
    #[clap(
        long = "input", value_enum, ignore_case = true, default_value_t = PrintBytecodeInputChoice::Script
    )]
