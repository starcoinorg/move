...
module Std::String {
    ...
    public fun utf8(bytes: vector<u8>): String {
        assert!(native_check_utf8(&bytes), EINVALID_UTF8);
        String{bytes}
    }

    /// Tries to create a new string from a sequence of bytes.
    public fun try_utf8(bytes: vector<u8>): Option<String> {
        if (native_check_utf8(&bytes)) {
    ...
    public fun insert(s: &mut String, at: u64, o: String) {
        let bytes = &s.bytes;
        assert!(at <= Vector::length(bytes) && native_is_char_boundary(bytes, at), EINVALID_INDEX);
        let l = length(s);
    ...
    public fun sub_string(s: &String, i: u64, j: u64): String {
        ...
        assert!(
            j <= l && i <= j && native_is_char_boundary(bytes, i) && native_is_char_boundary(bytes, j),
            EINVALID_INDEX
        );
        String{bytes: native_sub_string(bytes, i, j)}
    }
    ...
    public fun index_of(s: &String, r: &String): u64 {
        native_index_of(&s.bytes, &r.bytes)
    }


    // Native API
    native fun native_check_utf8(v: &vector<u8>): bool;
    native fun native_is_char_boundary(v: &vector<u8>, i: u64): bool;
    native fun native_sub_string(v: &vector<u8>, i: u64, j: u64): vector<u8>;
    native fun native_index_of(v: &vector<u8>, r: &vector<u8>): u64;
}